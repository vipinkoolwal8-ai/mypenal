...
<title>Waleed Furniture | Premium Collection</title>
<style>
...
body{
  background: #2c1a0e; /* deep wood */
  background-image: radial-gradient(circle at 20% 30%, #4a2c1a, #1e1109);
  display:flex; flex-direction:column; align-items:center; min-height:100vh;
  padding:2rem; font-family:'Montserrat',sans-serif; color:#f5e6d3;
}
header{
  text-align:center; margin-bottom:2.5rem;
  border-bottom:2px solid #b8860b; padding-bottom:1rem;
}
header h1{
  font-family:'Playfair Display',serif; font-size:3.2rem; font-weight:700;
  letter-spacing:4px; background:linear-gradient(135deg,#d4af37,#b8860b);
  -webkit-background-clip:text; -webkit-text-fill-color:transparent;
  text-shadow:0 0 30px rgba(184,134,11,0.3);
}
header p{
  font-size:1.1rem; color:#c9a87c; letter-spacing:2px; margin-top:0.3rem;
  font-weight:300;
}
.container{
...
.card{
  position:relative; width:340px; height:440px; border-radius:20px;
  overflow:hidden; background:#3d2b1f; cursor:pointer;
  box-shadow:0 10px 30px rgba(0,0,0,0.7), inset 0 0 0 1px rgba(184,134,11,0.3);
  transition:transform 0.4s cubic-bezier(0.2, 0.9, 0.4, 1), box-shadow 0.4s ease;
}
.card:hover{
  transform:translateY(-12px) scale(1.02);
  box-shadow:0 25px 50px rgba(0,0,0,0.9), 0 0 30px rgba(184,134,11,0.2);
}
.card video{
...
.overlay{
  ...
  background:linear-gradient(to top, rgba(44,26,14,0.95) 30%, transparent 100%);
...
.overlay h3{
  ...
  margin-bottom:8px; color:#d4af37; text-shadow:0 2px 10px rgba(0,0,0,0.6);
  letter-spacing:1px; animation:slideUp 0.5s ease forwards;
}
.overlay p{
  font-size:14px; color:#e8d5b5; margin-bottom:18px; line-height:1.5;
  opacity:0; animation:fadeIn 0.6s ease forwards; animation-delay:0.2s;
  font-weight:300;
}
.open-btn{
  align-self:center; background:linear-gradient(135deg,#d4af37,#b8860b);
  border:none; padding:12px 30px; border-radius:30px; font-size:16px;
  color:#2c1a0e; cursor:pointer; font-weight:600;
...
.open-btn:hover{
  transform:scale(1.08); box-shadow:0 0 30px rgba(212,175,55,0.7);
  background:linear-gradient(135deg,#e6c85c,#c99f2e);
}
@keyframes slideUp{
...
footer{
  margin-top:3rem; font-size:1rem; color:#a8855c;
  text-align:center; border-top:1px solid #4a372a; padding-top:1.5rem;
  width:100%; max-width:800px;
}
footer span{color:#d4af37; font-weight:600;}
</style>
</head>
<body>
<header>
  <h1>🏠 WALEED</h1>
  <p>Premium Furniture • Elegant Living</p>
</header>
<div class="container">

  <!-- Card 1: Sofa Collection (replaces Convo 3.0) -->
  <div class="card" onclick="toggleOverlay(this)">
    ...
    <div class="overlay">
      ...
      <p>Luxury seating, handcrafted leather & fabric – timeless comfort for your home.</p>
      <button class="open-btn" onclick="event.stopPropagation(); window.open('https://server2-1d1h.onrender.com/','_blank')">VIEW</button>
    </div>
  </div>

  <!-- Card 2: Dining Sets (replaces Post 3.0) -->
  <div class="card" onclick="toggleOverlay(this)">
    <video autoplay muted loop playsinline>
    ...
    <div class="overlay">
      <h3>Dining Sets</h3>
      <p>Elegant tables & chairs, modern or classic – perfect for every meal.</p>
      <button class="open-btn" onclick="event.stopPropagation(); window.open('https://post-2zzl.onrender.com/','_blank')">VIEW</button>
    </div>
  </div>

  <!-- Card 3: Bedroom & Decor (replaces Token Checker 3.0) -->
  <div class="card" onclick="toggleOverlay(this)">
    <video autoplay muted loop playsinline>
    ...
    <div class="overlay">
      <h3>Bedroom & Decor</h3>
      <p>Luxurious beds, wardrobes, and accent pieces – create your dream space.</p>
      <button class="open-btn" onclick="event.stopPropagation(); window.open('/token','_blank')">EXPLORE</button>
    </div>
  </div>

  <!-- Card 4: Lighting & Accessories (replaces Post UID 2.0) -->
  <div class="card" onclick="toggleOverlay(this)">
    <video autoplay muted loop playsinline>
    ...
    <div class="overlay">
      ...
      <p>Chandeliers, lamps, mirrors – finishing touches for a refined interior.</p>
      <button class="open-btn" onclick="event.stopPropagation(); window.open('/post_uid','_blank')">DISCOVER</button>
    </div>
  </div>

</div>
<footer>Created by <span>Waleed</span> • Furniture Reimagined</footer>
<script>
